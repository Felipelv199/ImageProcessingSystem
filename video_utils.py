...
def extract_frames(video, frames_path):
    ...
    while success:
        cv2.imwrite("{}/{}.png".format(frames_path, count),
...
def join_frames(frames_path, output):
    ...
    for count in range(0, len(frames)):
        ...
        filename = "{}/{}.png".format(frames_path, count)
...
if __name__ == "__main__":
    ...
    parser.add_argument("video", default="video.mp4",
                        help="input or output video path")
    parser.add_argument("frames_path", default="frames",
                        help="frames path path")
